package b.laixuantam.myaarlibrary.widgets.recyclerviewenhanced;
    ...
    private boolean isIndependentViewClicked(MotionEvent motionEvent) {
        for (int i = 0; i < independentViews.size(); i++) {
            if (touchedView != null) {
                ...
                if (touchedView.findViewById(independentViews.get(i)) != null)
    ...
    private int getOptionViewID(MotionEvent motionEvent) {
        for (int i = 0; i < optionViews.size(); i++) {
            if (touchedView != null) {
                ...
                int x = (int) motionEvent.getRawX();
                int y = (int) motionEvent.getRawY();
                if (touchedView.findViewById(optionViews.get(i)) != null)
    ...
    private void animateFadeViews(View downView, float alpha, long duration) {
        if (fadeViews != null) {
            for (final int viewID : fadeViews) {
                if (downView.findViewById(viewID) != null)
                    ...
                    if (deltaX < touchSlop && !bgVisible) {
                        ...
                        if (fadeViews != null) {
                            for (int viewID : fadeViews) {
                                if (touchedView.findViewById(viewID) != null)
