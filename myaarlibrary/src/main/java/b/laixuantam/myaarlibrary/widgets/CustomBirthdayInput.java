...
//    private Date maxDate;
    ...
    private Date minDate = stringToDate("01/01/1900", DEFAULT_DATE_FORMAT);
    private Date maxDate = stringToDate("31/12/2017", DEFAULT_DATE_FORMAT);


    //region CONSTRUCTORS
    public CustomBirthdayInput(Context context) {
        this(context, null);
    }

    public CustomBirthdayInput(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustomBirthdayInput(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        inflate(context, R.layout.layout_birthday_input, this);

        // Load the styled attributes and set their properties
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.BirthdayInput, defStyleAttr, 0);
                    ...
                    if (keyboardVisible != isVisible) {
                        ...
                        if (!keyboardVisible && editLazyDatePickerReal.isFocused()) {
                            ...
                            if (date.length() < LENGHT_DATE_COMPLETE && onDatePickListener != null) {
                                onDatePickListener.onCancelChange();
                            }
                        }
                    }
                }
    ...
    public boolean setDate(Date newDate) {
        ...
        editLazyDatePickerReal.setText(tmpDate);
